...
let publish = function
  | "distrib" :: uri :: name :: version :: msg :: archive :: _ ->
      let scp = Cmd.(v "echo" % "scp" % "-r" % docdir % (Printf.sprintf "web:/mnt/web/web/%s/distrib" name))
      in let _ = OS.Cmd.(run scp) in Ok 0
  | "doc" :: uri :: name :: version :: msg :: docdir :: _ ->
      let scp = Cmd.(v "echo" % "scp" % "-r" % (Printf.sprintf "%s/*" docdir) % (Printf.sprintf "web:/mnt/web/web/%s/doc" name))
      in let _ = OS.Cmd.(run scp) in Ok 0
